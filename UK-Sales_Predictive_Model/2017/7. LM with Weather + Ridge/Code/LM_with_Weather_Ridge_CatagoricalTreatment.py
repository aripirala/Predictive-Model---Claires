...
FILE_PATH_INPUT = 'C:/Users/aripiralas/GitHub/Predictive-Modeling/Predictive-Model---Claires/UK-Sales_Predictive_Model/2017/7. LM with Weather + Ridge/Input_Data/'

FILE_PATH_OUTPUT = 'C:/Users/aripiralas/GitHub/Predictive-Modeling/Predictive-Model---Claires/UK-Sales_Predictive_Model/2017/7. LM with Weather + Ridge/Output_Data/'
...
#################################################################################
### categorical multipliers function ####################

def getCatagoricalMultipliers(df):
   return df.mean()
...
######## convert holiday event and holiday period into onehotcode flags ###
...
data_df['Holiday_flag'] = data_df.Holiday_Event.apply(onehotcode)
data_df['Holiday_Period_flag'] = data_df.Holiday_Period.apply(onehotcode)

data_df.drop(['Holiday_Event','Holiday_Period'],inplace=True,axis=1)
###########################################################################
...
#zero_features_bool = [x not in ['Day_of_Week_2', 'Week_Num_18', 'Week_Num_19', 'Week_Num_22', 'Week_Num_38', 'Week_Num_47'] for x in list(zero_features)]  #### dont drop seasonality related columns
#
#
...
drop_catergorical_cols = ['Day_of_Week_0', 'Week_Num_1', 'Cluster_0']

print(data_dummy_df.shape)
data_dummy_df = data_dummy_df.drop(drop_catergorical_cols, axis=1)
print(data_dummy_df.shape)


############################################################################
####### get catagorical multipliers ########################################
multipliers_df = getCatagoricalMultipliers(data_dummy_df)
...
ridge_output_df.to_csv(FILE_PATH_OUTPUT+'Ridge_Weather_Output_catagorical_treatment.csv',index=False)
