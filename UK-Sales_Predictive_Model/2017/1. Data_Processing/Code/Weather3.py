...
def get_weather(start_date, end_date, city, state, country, weather_df):
    ...
    while d <= end_date:
        ...
        if day_counter == DAY_MAX_COUNT:
            day_counter = 0
            time.sleep(60 * 60 * 20)  ### sleep for 24 hours
...
for code in uk_airport_df_continue['Airport_Code']:
    ...
    weather_df.to_csv(FILE_PATH_LOCAL + 'JAN_weather_' + str(code) + '.csv')
...
raw_data = pd.DataFrame(columns=dump.columns)
# read and merge all csv in a for loop
for f in allfiles:
    raw_data = pd.merge(raw_data, pd.read_csv(f, header=0), how='outer')
# rearrange columns to fit data_processing.py
ready_data = raw_data[['0', '1', '4', '5', '18', '20', '24','29', '56']]
