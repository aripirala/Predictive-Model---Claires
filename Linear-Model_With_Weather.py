...
weather_df.rename(columns = {'City':'Airport_Code', 'Date':'Transaction_Date'}, inplace=True) ## rename city and Date columns to match rest of the dataframes

interested_weather_cols = ['Airport_Code', 'Transaction_Date',  'rain', 'snow','meantempi', 'precipi']

#### convert Transaction Date to datetime object ####


weather_df['Transaction_Date'] = pd.to_datetime(weather_df['Transaction_Date'],format='%m/%d/%Y')


store_airportCode_mapping_df = pd.read_csv(FILE_PATH+'Branch_AirportCode_mapping.csv')

#### convert store number column to category ####




sales_storeHours_hol_promo_cluster_weather_df = pd.merge(sales_storeHours_hol_promo_cluster_df, store_airportCode_mapping_df,
                 how='inner', on=['Store_Number'])

sales_storeHours_hol_promo_cluster_weather_df = pd.merge(sales_storeHours_hol_promo_cluster_weather_df, weather_df[interested_weather_cols],
                 how='left', on=['Airport_Code','Transaction_Date'])
